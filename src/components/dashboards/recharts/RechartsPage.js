  ...
  return (
    <>
      <div className="scrolling-wrapper-flexbox" style={{marginBottom: 16}}>
        <MenuButton className="card" value = "Простые графики" statusBtn={state.activeTabKey === "1"} onClick={() => {setState({...state, activeTabKey: "1", update: state.update + 1});}}/>
        <MenuButton className="card" value = "Динамические графики" statusBtn={state.activeTabKey === "2"} onClick={() => {setState({...state, activeTabKey: "2", update: state.update + 1});}}/>
